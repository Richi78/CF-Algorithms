...
int query(int node, int l, int r, int query_l, int query_r){
    ...
    if(r<query_l || query_r<l) return -1; 
    int mid=l+(r-l)/2;
    return query(node*2, l, mid, query_l, query_r)
        & query(node*2+1, mid+1, r, query_l, query_r);
}

void or_range(int node, int l, int r, int query_l, int query_r, int val){
    push(node, l ,r);
    ...
    int mid=l+(r-l)/2;
    or_range(node*2, l, mid, query_l, query_r, val);
    or_range(node*2+1, mid+1, r, query_l, query_r, val);
